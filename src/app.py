...
class App(tk.Tk):
        ...
        def _background():
            ...
            try:
                ...
                self.after(
                    ...
                    lambda: self.lbl_status.config(
                        text="状態: サムネイル生成中...", fg="#ffa500"
                    ),
                )
            except Exception as exc:
                logger.warning("サムネイル生成中ラベル更新に失敗しました: %s", exc)
            ...
            try:
                ...
                self.after(
                    0,
                    lambda: self.lbl_status.config(
                        text="状態: YouTubeへアップロード中...", fg="#ffa500"
                    ),
                )
            except Exception as exc:
                logger.warning("アップロード中ラベル更新に失敗しました: %s", exc)
    ...
    def _check_obs_connection(self):
        ...
        def _worker():
            try:
                ...
                status = self.obs_client.get_virtual_cam_status()
    ...
    def _on_upload_done(self, youtube_url: str):
        ...
        self.lbl_status.bind(
            "<Button-1>",
            lambda _: webbrowser.open(youtube_url),
        )
        messagebox.showinfo(
            "アップロード完了",
    ...
    def _on_upload_error(self, video_path: str):
        ...
        self._show_upload_error_dialog(video_path)

    def _show_upload_error_dialog(self, video_path: str):
        """アップロード失敗時のカスタムダイアログを表示する。"""
        dialog = tk.Toplevel(self)
        dialog.title("アップロード失敗")
        dialog.resizable(False, False)

        tk.Label(
            dialog,
            text=f"アップロードに失敗しました。\n録画ファイルは {video_path} に保存されています。",
            padx=16,
            pady=12,
            justify="left",
        ).pack()
